...
using TelemetryStorageServer.Data;
...
namespace TelemetryStorageServer.Services
{
    public class TelemetryPacketsService
    {
        ...
        public TelemetryPacket Unpack(byte[] package)
        {
            ...
            var telemetryPacketForRead = new TelemetryPacket();

            try
            {
                using (var readStream = new MemoryStream(package))
                {
                    using (var binaryReader = new BinaryReader(readStream))
                    {
                        ...
                        if (packagePrefix.Equals(packageWithTypesPrefix))
                        {
                            ...
                            if (typePrefix == telemetryTypePrefix) ReadTelemetryPacket(binaryReader, 
                                                                                       out telemetryPacketForRead);
                        }
                    }
                }
